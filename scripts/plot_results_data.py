...
def main():
    ...
    ax1.set_xscale('log')
    ax1.set_xlabel("Data noise factor")
    ax1.set_ylabel("Mean Position Error")
    ax1.set_title("Mean Position Error vs Data Noise")
    ax1.grid(True)

    # Mean ANEES
    ax2.errorbar(x, mean_anees, yerr=std_anees, marker='s', capsize=5)
    ax2.set_xscale('log')
    ax2.set_xlabel("Data noise factor")
    ax2.set_ylabel("Mean ANEES")
    ax2.set_title("Mean ANEES vs Data Noise")
    ax2.grid(True)
